...
pub fn puzzle1(input: &str) -> String {
    input.lines()
        ...
        .map(|(pack1, pack2)| pack1.chars().find_map(|item| if pack2.contains(item) {Some(item)} else {None}).unwrap())
        .map(char_to_num)
        .sum::<u16>()
        .to_string()
}
...
pub fn puzzle2(input: &str) -> String {
    input.lines().array_chunks::<3>()
        .map(|chunk| chunk[0].chars().find_map(|item| if chunk[1].contains(item) && chunk[2].contains(item) {Some(item)} else {None}).unwrap())
        .map(char_to_num)
        .sum::<u16>()
        .to_string()
}
